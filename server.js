...
app.put('/api/characters', function (req, res, next) {
  ...
  // First parallel database call to confirm both voted characters are valid
  async.parallel([
    function (callback) {
    ...
    // Second nested parallel database call - this time to save votes
    async.parallel([
      function (callback) {
        winner.wins++;
        winner.voted = true;
 ...
 * Return 100 highest ranked characters. Filter by gender.
 */
...
app.post('/api/report', function (req, res, next) {
  ...
  Character.findOne({ characterId: characterId }, function (err, character) {
    ...
    if (character.reports > 4) {
      character.remove();
      return res.send({ message: character.name + 'has been removed.' });
    }

    character.save(function (err) {
      if (err) return next(err);
      res.send({ message: character.name + ' has been reported. They have ' + character.reports + ' reports so far.' });
    });
  });
});
  ...
  async.parallel([
    ...
    function (callback) {
      Character.aggregate({ $group: { _id: null, total: { $sum: '$wins' } } }, function (err, totalVotes) {
        var total = totalVotes.length ? totalVotes[0].totaal : 0;
        callback(err, total);
      }
      );
    },
